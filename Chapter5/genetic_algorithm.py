...
class GeneticAlgorithm(Generic[C]):
    ...
    def _pick_tournament(self, num_participants: int, discard_top: int = 0) -> Tuple[C, C]:
        participants: List[C] = choices(self._population, k=num_participants)
        return tuple(nlargest(2 + discard_top, participants, key=self._fitness_key)[discard_top:])
    ...
    def _reproduce_and_replace(self) -> None:
        ...
        while len(new_population) < len(self._population):
            ...
            elif self._selection_type == GeneticAlgorithm.SelectionType.TOURNAMENT_2ND_BEST:
                parents = self._pick_tournament(len(self._population) // 2, 1)
            elif self._selection_type == GeneticAlgorithm.SelectionType.TOURNAMENT_3RD_BEST:
                parents = self._pick_tournament(len(self._population) // 2, 2)
            else:
